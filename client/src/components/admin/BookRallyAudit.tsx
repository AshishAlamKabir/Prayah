...
const transactionSchema = z.object({
  transactionType: z.enum(["sale", "purchase", "expense", "refund"]),
  category: z.enum(["book_sale", "book_purchase", "logistics", "promotion", "refund"]),
...
interface BookRallyTransaction {
  id: number;
...
export default function BookRallyAudit() {
  ...
  // Fetch book rally transactions
  const { data: transactions = [], isLoading } = useQuery<BookRallyTransaction[]>({
    queryKey: ["/api/book-rally-audit"],
  });

  // Calculate statistics
  const stats = {
    totalRevenue: transactions
      .filter((t: BookRallyTransaction) => t.transactionType === "sale")
      .reduce((sum: number, t: BookRallyTransaction) => sum + parseFloat(t.amount), 0),
    totalExpenses: transactions
      .filter((t: BookRallyTransaction) => ["purchase", "expense"].includes(t.transactionType))
      .reduce((sum: number, t: BookRallyTransaction) => sum + parseFloat(t.amount), 0),
    totalTransactions: transactions.length,
    unverifiedTransactions: transactions.filter((t: BookRallyTransaction) => !t.isVerified).length,
    netProfit: 0,
  };
  ...
  // Filter transactions
  const filteredTransactions = transactions.filter((transaction: BookRallyTransaction) => {
    const matchesSearch = 
  ...
  const addTransactionMutation = useMutation({
    mutationFn: async (data: TransactionFormData) => {
      const response = await apiRequest("POST", "/api/book-rally-audit", data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/book-rally-audit"] });
      toast({
        title: "Success",
  ...
  const verifyTransactionMutation = useMutation({
    mutationFn: async (transactionId: number) => {
      const response = await apiRequest("PATCH", `/api/book-rally-audit/${transactionId}/verify`);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/book-rally-audit"] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Book Rally Audit</h2>
          <p className="text-muted-foreground">
            Track all financial transactions for the book rally program
          </p>
        </div>
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Record a new financial transaction for the book rally
              </DialogDescription>
            </DialogHeader>
            <Form {...form}>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>
                            <SelectItem value="book_sale">Book Sale</SelectItem>
                            <SelectItem value="book_purchase">Book Purchase</SelectItem>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Search by description, vendor, customer, or invoice..."
                  value={searchTerm}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            All book rally financial transactions ({filteredTransactions.length} records)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {filteredTransactions.map((transaction: BookRallyTransaction) => (
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell>
                        <div className={`font-semibold ${
                          ...
                          {transaction.transactionType === "sale" ? "+" : "-"}₹{parseFloat(transaction.amount).toLocaleString()}
                        </div>
