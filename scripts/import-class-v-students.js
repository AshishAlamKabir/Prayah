...
/**
 * Script to import Class V student data from CSV file into BJV
 * This script processes the V_1754676300113.csv file and imports students
 */
...
function cleanStudentData(row, schoolId = 1) {
  // Build full name
  ...
  // Handle contact number (convert scientific notation)
  let contactNumber = row.mobile_no || row.father_phone || row.mother_phone || '';
  ...
  if (row.date_of_birth && row.date_of_birth !== '' && row.date_of_birth !== 'xxx') {
    try {
      const dateStr = row.date_of_birth.toString();
      if (dateStr.includes('/')) {
        const [day, month, year] = dateStr.split('/');
        dateOfBirth = new Date(parseInt(year), parseInt(month) - 1, parseInt(day)).toISOString();
      }
    } catch (error) {
      dateOfBirth = null;
    }
  ...
  // Parent name
  const parentName = row.father_name || row.mother_name || '';
  
  // Address
  const address = row.current_address || row.permanent_address || '';
  
  return {
    schoolId,
    name: fullName,
    rollNumber: row.roll_no ? row.roll_no.toString() : '',
    className: 'V', // Class V
    stream: null,
    ...
    dateOfBirth,
    gender: row.gender ? (row.gender.toLowerCase().includes('male') ? 'male' : 'female') : null,
...
async function importStudents() {
  try {
    console.log('🔄 Starting Class V student data import for BJV...');
    
    const students = parseCSV(csvData);
    console.log(\`📊 Found \${students.length} students in CSV data\`);
    ...
    for (let i = 0; i < students.length; i++) {
      try {
        ...
        const studentData = cleanStudentData(row, 1); // BJV school ID = 1
        ...
        // Make API request to add student
        const response = await fetch('http://localhost:5000/api/schools/1/students', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + 'temp-auth-token' // Would need real auth in production
          },
        ...
        if (response.ok) {
          successCount++;
          console.log(\`✅ Added student: \${studentData.name} (Roll: \${studentData.rollNumber})\`);
        } else {
          const error = await response.text();
          errorCount++;
          errors.push(\`Row \${i + 2}: \${studentData.name} - \${error}\`);
          console.log(\`❌ Failed to add \${studentData.name}: \${error}\`);
        }
      } catch (error) {
        errorCount++;
...
module.exports = { importStudents, parseCSV, cleanStudentData };