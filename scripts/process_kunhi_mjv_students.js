...
async function processKunhiStudents() {
  console.log('Processing Kunhi class students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/Kunhi_1754912810730.xlsx');
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      const studentInfo = {
        ...
        className: 'Kunhi', // Set class as Kunhi
    ...
    for (const student of studentData) {
      try {
        ...
        // Skip date processing for now to avoid insertion errors
    ...
    const finalCount = await db
      .select()
      .from(students)
      .where(eq(students.className, 'Kunhi'))
      .where(eq(students.schoolId, 3));
      
    console.log(`\n🎯 Total Kunhi class students in MJV: ${finalCount.length}`);
    console.log('Kunhi student import completed');
    
  } catch (error) {
    console.error('Error processing Kunhi students:', error);
  }
}

processKunhiStudents();