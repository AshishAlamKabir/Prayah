...
async function processClass5Students() {
  console.log('Processing Class 5 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 5_1754913892031.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 5 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'V'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class V students in MJV: ${finalCount.length}`);
    console.log('Class 5 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 5 students:', error);
  }
}

processClass5Students();