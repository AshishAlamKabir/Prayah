...
async function processClass1Students() {
  console.log('Processing Class 1 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 1_1754912989803.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 1 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'I'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class I students in MJV: ${finalCount.length}`);
    console.log('Class 1 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 1 students:', error);
  }
}

processClass1Students();