...
async function processClass8Students() {
  console.log('Processing Class 8 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 8_1754913926545.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 8 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'VIII'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class VIII students in MJV: ${finalCount.length}`);
    console.log('Class 8 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 8 students:', error);
  }
}

processClass8Students();