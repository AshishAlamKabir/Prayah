...
async function processClass6Students() {
  console.log('Processing Class 6 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 6_1754913904468.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 6 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'VI'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class VI students in MJV: ${finalCount.length}`);
    console.log('Class 6 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 6 students:', error);
  }
}

processClass6Students();