...
async function processClass4Students() {
  console.log('Processing Class 4 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 4_1754913720628.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 4 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'IV'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class IV students in MJV: ${finalCount.length}`);
    console.log('Class 4 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 4 students:', error);
  }
}

processClass4Students();