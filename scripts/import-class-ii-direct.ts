...
async function importStudents(): Promise<void> {
  try {
    console.log('🔄 Starting direct import of Class II students for BJV...');
    
    // Read Excel file
    const excelPath = path.join(process.cwd(), 'attached_assets', 'II_1754717271427.xlsx');
...
async function main() {
  try {
    ...
    console.log('\n🎉 Class II student import completed!');
    process.exit(0);
  } catch (error) {
    console.error('💥 Import failed:', error);
    process.exit(1);
