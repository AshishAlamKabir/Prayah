...
async function importStudents(): Promise<void> {
  try {
    console.log('🔄 Starting direct import of Class VII students for BJV...');
    
    // Read Excel file
    const excelPath = path.join(process.cwd(), 'attached_assets', 'VII_1754717420478.xlsx');
...
async function main() {
  try {
    ...
    console.log('\n🎉 Class VII student import completed!');
    process.exit(0);
  } catch (error) {
    console.error('💥 Import failed:', error);
    process.exit(1);
