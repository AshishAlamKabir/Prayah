...
async function importStudents(): Promise<void> {
  try {
    console.log('🔄 Starting direct import of Class XII Science students for BJV...');
    
    // Read Excel file
    const excelPath = path.join(process.cwd(), 'attached_assets', 'XII Science_1754717560193.xlsx');
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // For Science stream, add 200 to avoid conflicts with Arts and Commerce streams
        const baseRollNumber = safeString(
          row.roll_no || row.rollno || row.roll_number || row.roll || (i + 1)
        );
        const rollNumber = String(parseInt(baseRollNumber) + 200);
...
async function main() {
  try {
    ...
    console.log('\n🎉 Class XII Science student import completed!');
    process.exit(0);
  } catch (error) {
    console.error('💥 Import failed:', error);
    process.exit(1);
