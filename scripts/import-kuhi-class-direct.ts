...
async function importStudents(): Promise<void> {
  try {
    console.log('🔄 Starting direct import of Kuhi class students for BJV...');
    
    // Read Excel file
    const excelPath = path.join(process.cwd(), 'attached_assets', 'Kuhi_1754715273263.xlsx');
...
async function main() {
  try {
    ...
    console.log('\n🎉 Kuhi class student import completed!');
    process.exit(0);
  } catch (error) {
    console.error('💥 Import failed:', error);
    process.exit(1);
