...
async function importStudents(): Promise<void> {
  try {
    console.log('🔄 Starting direct import of Class XII Commerce students for BJV...');
    
    // Read Excel file
    const excelPath = path.join(process.cwd(), 'attached_assets', 'XII Commerce_1754717549968.xlsx');
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // For Commerce stream, add 100 to avoid conflicts with Arts stream roll numbers
        const baseRollNumber = safeString(
          row.roll_no || row.rollno || row.roll_number || row.roll || (i + 1)
        );
        const rollNumber = String(parseInt(baseRollNumber) + 100);
...
async function main() {
  try {
    ...
    console.log('\n🎉 Class XII Commerce student import completed!');
    process.exit(0);
  } catch (error) {
    console.error('💥 Import failed:', error);
    process.exit(1);
