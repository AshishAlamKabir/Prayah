...
async function processAnkurExcel() {
  try {
    const excelFile = path.join(__dirname, '..', 'attached_assets', 'Shupan_1754753496674.xlsx');
    ...
    for (let rowNum = 2; rowNum <= worksheet.rowCount; rowNum++) {
      ...
      if (row.hasValues) {
        ...
        const student = {
          ...
          admissionNumber: extractValue(rowData, ['Admission No', 'ভৰ্তি নং', 'Admission']) || `SOP${String(rowNum - 1).padStart(3, '0')}`,
...
async function main() {
  console.log('SOPAN Student Data Processor');
  ...
  if (students && students.length > 0) {
    ...
    const backupFile = path.join(__dirname, 'sopan_students_backup.json');
