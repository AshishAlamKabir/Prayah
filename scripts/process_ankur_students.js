...
async function processAnkurExcel() {
  try {
    const excelFile = path.join(__dirname, '..', 'attached_assets', 'class 5_1754753661974.xlsx');
    ...
    for (let rowNum = 2; rowNum <= worksheet.rowCount; rowNum++) {
      ...
      if (row.hasValues) {
        ...
        const student = {
          ...
          admissionNumber: extractValue(rowData, ['Admission No', 'ভৰ্তি নং', 'Admission']) || `C5${String(rowNum - 1).padStart(3, '0')}`,
...
async function main() {
  console.log('Class V Student Data Processor');
  ...
  if (students && students.length > 0) {
    ...
    const backupFile = path.join(__dirname, 'class5_students_backup.json');
