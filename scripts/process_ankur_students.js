...
async function processAnkurExcel() {
  try {
    const excelFile = path.join(__dirname, '..', 'attached_assets', 'class 7_1754753702115.xlsx');
    ...
    for (let rowNum = 2; rowNum <= worksheet.rowCount; rowNum++) {
      ...
      if (row.hasValues) {
        ...
        const student = {
          ...
          admissionNumber: extractValue(rowData, ['Admission No', 'ভৰ্তি নং', 'Admission']) || `C7${String(rowNum - 1).padStart(3, '0')}`,
...
async function main() {
  console.log('Class VII Student Data Processor');
  ...
  if (students && students.length > 0) {
    ...
    const backupFile = path.join(__dirname, 'class7_students_backup.json');
