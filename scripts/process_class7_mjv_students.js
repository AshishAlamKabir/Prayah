...
async function processClass7Students() {
  console.log('Processing Class 7 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 7_1754913916220.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 7 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'VII'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class VII students in MJV: ${finalCount.length}`);
    console.log('Class 7 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 7 students:', error);
  }
}

processClass7Students();