...
async function processClass3Students() {
  console.log('Processing Class 3 students for MJV...');

  try {
    console.log('Reading Excel data...');
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('attached_assets/class 3_1754913881376.xlsx');
    ...
    console.log(`Found ${studentData.length} Class 3 students to process`);
    ...
    const finalCount = await db
      ...
      .from(students)
      .where(and(
        eq(students.className, 'III'),
        eq(students.schoolId, 3)
      ));
      
    console.log(`\n🎯 Total Class III students in MJV: ${finalCount.length}`);
    console.log('Class 3 student import completed');
    
  } catch (error) {
    console.error('Error processing Class 3 students:', error);
  }
}

processClass3Students();